    ...
    <div className="p-8 pb-24 md:p-10 max-w-6xl mx-auto h-full overflow-y-auto no-scrollbar relative animate-in fade-in duration-300">
      <div className="mb-10">
        <ViewHeader
          ...
          badge={<span className="bg-primary/20 text-primary border border-primary/30 px-2 py-0.5 rounded-md text-[10px] font-mono tracking-widest uppercase">Repo &amp; Process</span>}
          actions={
            <div className="flex flex-col items-end">
              <div className="text-[10px] font-bold text-success uppercase tracking-widest flex items-center gap-2">
          ...
          <div className="panel p-6">
            ...
            <div className="flex flex-col gap-3">
              {!loaded ? (
                ...
                <div className="text-xs font-mono text-muted uppercase p-4 text-center border border-dashed border-border rounded-lg">No tracked repositories</div>
                ...
                <div key={repo.id} style={{ animationDelay: `${Math.min(i, 8) * 45}ms` }} className="flex flex-col md:flex-row justify-between md:items-center p-4 bg-background/50 border border-border rounded-lg gap-4 card-lift animate-in fade-in slide-in-from-bottom-1 fill-mode-backwards">
                  ...
                  <div className="flex flex-wrap md:flex-nowrap items-center gap-3">
                    {repo.risk.length > 0 && (
                      <span className="text-[10px] font-mono px-2 py-1 bg-danger/10 border border-danger/20 text-danger font-bold rounded">
                        {repo.risk.length} RISK{repo.risk.length > 1 ? 'S' : ''}
                      </span>
                    )}
                    {repo.outOfSync ? (
                      <span className="text-[10px] font-mono px-2 py-1 bg-warning/10 border border-warning/20 text-warning font-bold rounded">
                        OUT OF SYNC
                      </span>
                    ) : (
                      <span className="text-[10px] font-mono px-2 py-1 bg-success/10 border border-success/20 text-success font-bold rounded">
                        SYNCED
                      </span>
                    )}
                  </div>
              ...
              {!loaded ? (
                ...
                <div key={bin.id} style={{ animationDelay: `${Math.min(i, 8) * 45}ms` }} className="p-4 rounded-lg border bg-background/50 border-border relative overflow-hidden card-lift animate-in fade-in slide-in-from-bottom-1 fill-mode-backwards">
                  <div className="flex justify-between items-start mb-3">
                    <span className="text-sm font-bold text-text flex items-center gap-2 min-w-0">
              ...
              <div key={a.key} style={{ animationDelay: `${Math.min(i, 10) * 40}ms` }} className="flex flex-col gap-1 border-b border-border/40 pb-3 last:border-0 animate-in fade-in slide-in-from-right-1 fill-mode-backwards">
                <div className="flex items-center justify-between text-[9px] text-muted">
                  ...
                  <span className={`px-1.5 py-0.5 rounded uppercase font-bold flex items-center gap-1
                    ${a.level === 'CRITICAL' ? 'bg-danger/20 text-danger' : 'bg-warning/20 text-warning'}`}>
