# Onyx Architecture

## High-Level Topology

The Onyx app is a Monorepo using npm workspaces.

```text
/
├── packages/
│   ├── ui/       # React + Vite (Frontend)
│   ├── core/     # Electron Main Process & IPC Preload
│   └── tools/    # Pure Node.js utilities (Networking, Git)
├── package.json
└── README.md
```

## Security & IPC Boundaries
- **Context Isolation** is fully enabled in Electron.
- The UI runs in a sandboxed generic Web Context with zero native Node.js privileges.
- Inter-Process Communication (IPC) is handled explicitly through `@electron/contextBridge` which exposes `window.api`.

### Development Fallback (Mock API)
In local development (running `npm run dev` in standard Chrome/Firefox without Electron runtime attached), `window.api` does not natively exist.
To solve this, `packages/ui/src/main.tsx` calls `injectMockApi()` before the React app mounts, which attaches `window.api` only when it is absent (i.e. outside Electron). This permits full UI, CSS layout, and state logic development directly in the browser.

## Plugin system (signed extensions)
Third-party functionality ships as **signed plugin bundles**, never as arbitrary code:
- **Signature is the trust root.** Each bundle is signed offline with the Onyx Ed25519 private key (`infra/plugin-signing/`); the app ships only the public key (`packages/core/src/plugins/public-key.js`) and verifies every bundle *before requiring a single line*. Verification **fails closed** — no key, or a bad/altered signature, means nothing loads.
- **Capability model.** A plugin's manifest declares the permissions it needs from a **closed catalog** (`packages/core/src/plugins/permissions.js`). The user approves them, and the host API handed to the plugin exposes *only* the approved capabilities.
- **Kill-switch (revocation).** A signed plugin later found malicious/broken can be revoked: the runtime fetches a sticky revocation list from the registry (`revoked.json`), and a revoked id is never activated, can't be re-enabled or re-installed, and is surfaced as "Revoked" in the UI. Sticky = once revoked, stays revoked even offline (the cached set only grows). This is the lightweight companion to the (still pending) real sandbox.
- **Honest trust boundary (current limitation).** A verified plugin's backend is `require()`d into the **main process**, so it runs in-process with full Node access — the capability layer is **least-privilege over the host API + transparency for review**, *not* a hard sandbox. The real boundary is therefore the **signature**: Onyx only signs code it has reviewed. UI copy reflects this ("reviewed & signed · here's what it declares", never "it can only ever touch this"). Making capabilities *enforced* (running plugin backends in a Node-less `utilityProcess`/worker that talks only over the approved channels) is the planned next step before accepting third-party plugins at scale; a revocation list is the companion kill-switch.
- **Install is a two-step consent.** `plugin:pickBundle` opens a folder picker, verifies the signature, and returns just the manifest preview — keeping the source path in the main process so the renderer can never drive an arbitrary copy. The Extensions view then shows a consent modal listing every requested capability by risk tier; only on approval does `plugin:install` copy the *pre-verified* bundle in, granting at most what was declared (`granted ⊆ permissions`). Nothing unverified ever reaches `userData/plugins/`.
- **In-app registry install.** `plugin:registryList` / `plugin:registryPreview` fetch from a **hardcoded, trusted** registry (`raw.githubusercontent.com/Sepd0x/onyx-plugins`, never user-supplied → no SSRF), bounded to the `plugins/<id>` tree. `registryPreview` downloads the manifest + signature + referenced files, runs the SAME `loadBundle` verification, and stashes the verified *bytes* as the pending install — feeding the identical consent modal. `plugin:install` then writes those verified bytes. So a downloaded plugin is signature-checked before it ever touches disk; the network path widens convenience, not trust.
- **Single gated channel.** Plugins never register their own IPC channels. The renderer reaches plugin code only through `plugin:invoke`, gated by: installed → verified → enabled → method declared in the manifest. The core channel allowlist stays closed and `channels.test.js` keeps enforcing it.
- Verified plugins load from `userData/plugins/`; the runtime lives in `packages/tools/plugins/`. See `infra/plugin-signing/README.md` to sign one, and the curated registry at `Sepd0x/onyx-plugins`.

## Themes
The application styling natively bridges Tailwind utility classes with core CSS variables (see `index.css`).
Themes switch dynamically by setting `data-theme` attribute on the root HTML element.
